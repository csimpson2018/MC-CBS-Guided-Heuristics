...
#include <nlohmann/json.hpp> // writeJSON, record_*_Node()
...
void CBS::saveResults(const string &fileName, const string &instanceName)
{
	...
	if (!exist)
	{
		...
		addHeads << "runtime,#high-level expanded,#high-level generated,#low-level expanded,#low-level generated," <<
			"solution cost,sum of costs,min f value,root g value, root f value," <<
			"#adopt bypasses," <<
			"cardinal conflicts," <<
	...
	stats << runtime << "," << 
		...
		solution_cost << "," << sum_of_costs << "," << cost_lowerbound << "," << dummy_start->g_val << "," <<
...
void CBS::writeJSON()
{
	...
	if(boost::filesystem::exists(json_file_name))
	{
		...
		std::unordered_map<int, int> json_cost_dead_count_map = jsonToWrite["costDeads"].get<std::unordered_map<int, int>>();

		std::vector<int> json_solutionCosts = jsonToWrite["solutionCosts"].get<std::vector<int>>();

		json_solutionCosts.push_back(solution_cost);
	...
	if (solutionCosts.size() == 0)
	{
		solutionCosts.push_back(solution_cost);
	}
	...
	jsonToWrite["costDeads"] = cost_dead_count_map;
...
void CBS::recordGoalNode(const HLNode* node)
{

	if (screen > 0)
	std::cout << "Recording goal node!" << std::endl;
	...
	cost_goal_count_map[solution_cost] += 1;
...
void CBS::recordDeadNode(const HLNode* node)
{

	std::cout << "Recording dead node!" << std::endl;

	try
	{
		levelDeadCounts.at(node->depth) += 1;
	}
	catch(const std::out_of_range& oor)
	{
		levelDeadCounts.push_back(1);
	}
...
void CBS::recordRegularNode(const HLNode* node)
{

	if (screen > 0)
		std::cout << "Recording regular node!" << std::endl;
...
CBS::CBS(const Instance& instance, bool sipp, int screen, std::string json_file_name) :
	screen(screen), suboptimality(1),
