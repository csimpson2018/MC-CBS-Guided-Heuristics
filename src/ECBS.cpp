...
bool ECBS::solve(double time_limit, int _cost_lowerbound)
{
	...
	while (!cleanup_list.empty() && !solution_found)
	{
		...
		if (bypass && (curr->chosen_from != "cleanup" || solver_type == high_level_solver_type::MCR))
		{
			...
			while (foundBypass)
			{
				...
				curr->conflict = chooseConflict(*curr); // Change this to if statement for batch conflicts
				addConstraints(curr, child[0], child[1]); // Change to batch constraints, same if-statement
				if (screen > 1)
				...
				for (int i = 0; i < 2; i++)
				{
					...
					solved[i] = generateChild(child[i], curr); // Will have to change this as well, for multiple constraints
					if (!solved[i])
					{
						delete (child[i]);
						continue;
				...
				{
					if (solver_type != high_level_solver_type::MCR)
					{
						for (int i = 0; i < 2; i++)
						{
							if (solved[i])
					...
					else // If solver type is MCR
					{

						if (guide_type == heuristic_guide::ZERO)
						{
						...
						else if (guide_type == heuristic_guide::SOFTMAX)
						{
							...
							vector<double> h_vals(2, -__DBL_MAX__);

							if (solved[0])
							{
								heuristic_helper.computeInformedHeuristics(*child[0], min_f_vals, time_limit - runtime);
								h_vals.at(0) = child[0]->h_val;
							}

							if (solved[1])
							{
								heuristic_helper.computeInformedHeuristics(*child[1], min_f_vals, time_limit - runtime);
								h_vals.at(1) = child[1]->h_val;
							}

							vector<double> weights = softmax(h_vals);

							for (int i = 0; i < weights.size(); i++)
							{
								if (chance < weights.at(i))
								{
									if (!solved[i])
									{
										recordDeadNode(child[i]);
										return solution_found;
									}
									pushNode(child[i]);
									curr->children.push_back(child[i]);
						...
						else if (guide_type == heuristic_guide::EPSILON)
						{
							double chance = dist(generator);
							...
							vector<double> weights = epsilon_greedy(h_vals);
		...
		if ( cleanup_list.empty() && !solution_found )
		{
			recordDeadNode(curr);
		}
...
bool ECBS::generateRoot()
{
	...
	getSOC(root, initial_constraints, agents);
...
inline bool ECBS::reinsertNode(ECBSNode* node)
{
	...
	case high_level_solver_type::EES:
		...
		if (node->getFHatVal() <= suboptimality * inadmissible_cost_lowerbound)
			node->focal_handle = focal_list.push(node);
		break;
	case high_level_solver_type::MCR:
		node->cleanup_handle = cleanup_list.push(node);
		break;
	default:
		break;
...
void ECBS::getSOC(ECBSNode* node, vector<ConstraintTable> initial_constraints, vector<int> agents)
{

	vector<Path> optimal_paths(agents.size());

	for (int i : agents)
	{
		optimal_paths[i] = search_engines[i]->findOptimalPath(*node, initial_constraints[i], paths, i, 0);
		if (optimal_paths[i].empty())
		{
			cerr << "The start-goal locations of agent " << i << "are not connected" << endl;
			exit(-1);
		}
		
		sum_of_costs += (int)optimal_paths[i].size() - 1;
	}
