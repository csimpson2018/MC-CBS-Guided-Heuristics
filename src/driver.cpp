...
int main(int argc, char** argv)
{
	...
	desc.add_options()
		...
		("jsonName,j", po::value<std::string>()->default_value("Null"), "Name of the JSON file to write to (include file extension)")
	...
	else if (vm["highLevelSolver"].as<string>() == "MCR")
		s = high_level_solver_type::MCR;
	else
	{
	...
	heuristic_guide g;
	if (vm["heuristicGuide"].as<string>() == "Zero")
		g = heuristic_guide::ZERO;
	else if (vm["heuristicGuide"].as<string>() == "Softmax")
    ...
    {
        ECBS ecbs(instance, vm["sipp"].as<bool>(), vm["screen"].as<int>(), vm["jsonName"].as<string>());
    ...
    {
        CBS cbs(instance, vm["sipp"].as<bool>(), vm["screen"].as<int>(), vm["jsonName"].as<string>());
