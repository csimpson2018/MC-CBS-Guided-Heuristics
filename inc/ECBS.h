...
{
public:
	ECBS(const Instance& instance, bool sipp, int screen, std::string json_file_name) : CBS(instance, sipp, screen, json_file_name) {}
...
private:
	...
	void getSOC(ECBSNode* node, vector<ConstraintTable> initial_constraints, vector<int> agents); // Intended to compute optimal(!) sum of costs for comparisons
